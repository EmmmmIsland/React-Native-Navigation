...
import {SectionList, Text, View, StyleSheet, Platform, TouchableOpacity} from 'react-native'
...
export  default  class ListThree extends Component {
    ...
    async getCityInfos() {
        let data = await require('../../area/citya.json');
    ...
    _renderItem = (item) => {
        ...
        <TouchableOpacity
            onPress={()=>{
                console.log(item.item.city_child,item.item.city_parent,item.item.provcn)
            }}
        >
